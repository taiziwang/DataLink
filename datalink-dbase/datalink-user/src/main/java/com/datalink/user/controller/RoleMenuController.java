...
public class RoleMenuController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "菜单角色ID", required = true, dataType = "int(11)"),
            @ApiImplicitParam(name = "roleId", value = "角色ID", required = true , dataType = "int(11)"),
            @ApiImplicitParam(name = "menuId", value = "菜单ID", required = true , dataType = "int(11)"),
    })
    @PostMapping("/saveOrUpdate")
    public Result saveOrUpdate(@RequestBody RoleMenu roleMenu) throws Exception {
        return roleMenuService.saveOrUpdateRoleMenu(roleMenu);
    }
    ...
    @PostMapping("/list")
    public PageResult<RoleMenu> listRoleMenus(@RequestBody JsonNode para) {
        return roleMenuService.selectForCTable(para);
    }
    ...
    public Result getOneById(@RequestBody RoleMenu roleMenu) throws Exception {
        roleMenu = roleMenuService.getById(roleMenu.getId());
        return Result.succeed(roleMenu,"获取成功");
    }
}
