...
public class DictController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "字典ID", required = true, dataType = "int(11)"),
            @ApiImplicitParam(name = "code", value = "字典编码", required = true , dataType = "varchar(255)"),
            @ApiImplicitParam(name = "name", value = "字典名", required = true , dataType = "varchar(255)"),
            @ApiImplicitParam(name = "type", value = "类型", required = true , dataType = "varchar(255)"),
            @ApiImplicitParam(name = "note", value = "说明", required =  false , dataType = "varchar(255)"),
            @ApiImplicitParam(name = "enabled", value = "启用", required = true , dataType = "tinyint(1)"),
            ...
            @ApiImplicitParam(name = "tenantId", value = "租户ID", required = true , dataType = "int(11)"),
    })
    @PostMapping("/saveOrUpdate")
    public Result saveOrUpdate(@RequestBody Dict dict) throws Exception {
    ...
    @PostMapping("/list")
    public PageResult<Dict> listDicts(@RequestBody JsonNode para) {
        return dictService.selectForCTable(para);
    }
    ...
    public Result getOneById(@RequestBody Dict dict) throws Exception {
        dict = dictService.getById(dict.getId());
        return Result.succeed(dict,"获取成功");
    }
}
